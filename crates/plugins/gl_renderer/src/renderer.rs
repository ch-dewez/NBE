...
use ecs::{
    query::Query,
    ressource::{Res, ResMut, Ressource},
    system::SystemParam,
};
use engine::plugin::Plugin;
use glam::Mat4;
use opengl::{
    CAMERA_BIND_INDEX, DEFAULT_UNIFORM_CAPACITY, MODEL_BIND_INDEX,
    context::OpenGlContext,
    glow::{self, HasContext},
    material::MaterialComponent,
    mesh::MeshComponent,
    uniform_buffer::{GrowableUniformBuffer, UniformBuffer},
};
use window::{
    glfw::Context,
    window::{GLFWRes, GLFWWindowRes},
};
...
fn camera_ubo_update(
    query: Query<(&Camera, &Transform)>,
    camera_ubo: Res<CameraUBO>,
    gl: Res<OpenGlContext>,
) {
...
fn render_system(
    query: Query<(&Transform, &MeshComponent, &MaterialComponent)>,
    gl: Res<OpenGlContext>,
    mut model_ubo: ResMut<ModelUBO>,
) {
    ...
    for (index, (transform, mesh, material)) in query.into_iter().enumerate() {
        model_ubo
            .0
            .update_data(&gl, index, &transform.get_world_matrix());
...
impl Plugin for GlRenderer {
    fn init<'a, 'b>(&self, context: engine::plugin::PluginContext<'a, 'b>) {
        ...
        let model_ubo = GrowableUniformBuffer::new(
            &gl,
            MODEL_BIND_INDEX,
            DEFAULT_UNIFORM_CAPACITY,
            size_of::<Mat4>(),
        );
        let camera_ubo =
            UniformBuffer::new::<CameraUboData>(&gl, CAMERA_BIND_INDEX, glow::DYNAMIC_DRAW);
        camera_ubo.bind(&gl);

        drop(gl);

        let mut glfw = ResMut::<GLFWRes>::retrieve_no_local(&context.application.world)
            .expect("Can't get GLFW");
